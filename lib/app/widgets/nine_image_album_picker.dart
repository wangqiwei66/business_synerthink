...
class SelectPhotoOrAlbumWidget extends StatefulWidget {
  ///每次點選選擇圖片後的回撥
  ...
  const SelectPhotoOrAlbumWidget({
    super.key,
    this.header,
    this.tips,
  ...
  State<StatefulWidget> createState() {
    return _SelectPhotoOrAlbumWidgetState();
  }
}

class _SelectPhotoOrAlbumWidgetState extends State<SelectPhotoOrAlbumWidget>
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          ...
          return widget.isDragable
              ? _buildDragAbleItem(imageUrl, index)
              : _buildItem(imageUrl, index);
        },
      ),
    );
  }
            ...
            onItemClickListener: (index) async {
              ...
              try {
                if (index == 0) {
                  ...
                  //拍照选择图片
                  XFile? souce = await ImageUtils().pickerImage(
                    source: PickerType.camera,
                    imageQuality: 40,
                  );
